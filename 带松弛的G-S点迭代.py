...
#函数：对隐式通用离散格式进行G-S迭代和松弛迭代的改造，求解未知迭代层的温度
def general_discrete_format(a_p0, a_e, a_w, a_n, a_s, T_e, T_w, T_n, T_s, T_p0, Q, w):
    a_p = a_p0 + a_e + a_n + a_w + a_s
    T_p_GS = (a_e * T_e + a_w * T_w + a_s * T_s + a_n * T_n + a_p0 * T_p0 + Q) / a_p
    T_p = (1 - w) * T_p0 + w * T_p_GS
    return float(T_p)

#函数：求解n * n个点的温度演化情况
def calculate_T(T, dx, dy, qw, qe, dt, delta_x, delta_y, n, w):
    ...
    for i in range(1, n-1):
        T_new[i][0] = general_discrete_format(a_p0=dx*dy/(2*dt), a_e=dy/dx, a_w=0.0, a_n=dx/(2*dy), a_s=dx/(2*dy), \
            T_e=T[i][1], T_w=0.0, T_n=T[i+1][0], T_s=T_new[i-1][0], T_p0=T[i][0], Q=qw*dy, w=w) #左边界，i=1到9, j=0
    
    for i in range(1, n-1):
        T_new[i][n-1] = general_discrete_format(a_p0=dx*dy/(2*dt), a_e=0.0, a_w=dy/dx, a_n=dx/(2*dy), a_s=dx/(2*dy), \
            T_e=0.0, T_w=T_new[i][n-2], T_n=T[i+1][n-1], T_s=T_new[i-1][n-1], T_p0=T[i][n-1], Q=qe*dy, w=w) #右边界，i=1到9, j=10

    for i in range(1, n-1):
        for j in range(1, n-1):
            T_new[i][j] = general_discrete_format(a_p0=dx*dy/dt, a_e=dy/dx, a_w=dy/dx, a_n=dx/dy, a_s=dx/dy, \
                T_e=T[i][j+1], T_w=T_new[i][j-1], T_n=T[i+1][j], T_s=T_new[i-1][j], T_p0=T[i][j], Q=0.0, w=w) #非边界点, i=2到9, j=2到9
...
# #调用上述函数
# while (t_total <= t_max):
#     T_iteration = np.copy(calculate_T(T, dx, dy, qw, qe, dt, delta_x, delta_y, n, w))
...
#         #输出
#         print('{0:*^90}'.format('带松弛的GS迭代平衡时间及迭代次数如下'))
...
#调用上述函数
while (w <= 2.0): #w查找范围为0.1到2.0，间隔0.1计算一次
    ...
    while (t_total <= t_max):
        ...
        if compare_restrain(T, T_now, epsilon_time, n) == 0: #判断是否达到稳态
            if np.isnan(T_now).any() == False: #判断数组是否出现nan，不出现才可以输出，出现nan的原因是因为判断相对变化时出现分母为0的情况
                ...
                dur = time.perf_counter() - start
                #输出
                print('{0:*^90}'.format('带松弛的G-S点迭代平衡时间及迭代次数如下'))
                print('传热达到平衡，程序耗时为{0:} sec'.format(dur), end='\n'*2)
                print('总迭代次数为{0:}次'.format(total_iteration), end='\n'*2)
                ...
                for i in range(n-1, -1, -1):#由于ndarray格式直接输出是下标小的数在前，所以需要把数组i方向下标倒序输出以更直观
                    for j in range(n):
                        print('{0: ^8}'.format(round(T_now[i][j], 4)), end=' ')
                    print('\n')
                if round(w, 1) == 1.8:
                    #作w=1.8时的稳定状态温度图
