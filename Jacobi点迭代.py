...
T_past = np.copy(T) #定义一个“篮子”，临时存放前一个已知时层计算好的结果
...
plt.xlabel('X方向')
plt.ylabel('Y方向')
picture = plt.contourf(np.linspace(0, 1, n),np.linspace(0, 1, n),T,n,cmap=plt.cm.hot) 
plt.title("初始时刻无量纲温度分布图")
plt.colorbar()
'''

#函数：根据隐式通用离散格式，求解未知迭代层的温度
...
while (t_total <= t_max):
    ...
    #判断是否达到稳态
    if compare_restrain(T_past, T_now, epsilon_time, n) == 0: #判断是否达到稳态
        #计时
        dur = time.perf_counter() - start
        #输出
        print('{0:*^90}'.format('Jacobi迭代平衡时间及迭代次数如下'))
